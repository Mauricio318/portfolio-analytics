...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#f9fafb' }}>
      <aside style={{ backgroundColor: '#111827', color: '#fff', padding: '1rem' }}>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '1rem', textAlign: 'center', fontWeight: 700 }}>Painel Admin</h2>
        <nav style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', justifyContent: 'center', alignItems: 'center' }}>
          ...
          <a href="/" style={{ color: '#aaa', textDecoration: 'none', padding: '0.6rem 0.9rem', borderLeft: '1px solid #374151', marginLeft: '0.5rem', whiteSpace: 'nowrap', fontSize: '0.9rem' }} target="_blank">Ver Site ↗</a>
          <form method="POST" action="/api/auth/logout" style={{ display: 'inline' }}>
            <button type="submit" style={{ padding: '0.6rem 1rem', backgroundColor: '#ef4444', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.9rem', whiteSpace: 'nowrap' }}>
              Sair
            </button>
          </form>
        </nav>
